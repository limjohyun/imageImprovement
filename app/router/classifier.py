...
실사진 QA로 추가 확인된 알려진 한계(수정 보류, `docs/roadmap.md` 해당 항목 참고):
오선/TAB 라인이 프렛 번호·빔·슬러 등으로 촘촘히 끊긴 악보(예: 우쿨렐레 TAB
편곡보)는 `_horizontal_line_mask`가 요구하는 연속 실행 길이를 채우지 못해
SCORE로 분류되지 않고 TEXT로 떨어진다. 반대로 등간격 표(격자) 문서는 셀
경계선이 끊기지 않은 완전한 가로선이라 오히려 SCORE로 오탐될 수 있음을 실험으로
확인했다 — 즉 오선 검출 민감도를 높이는 방향의 수정은 표 문서와의 오탐 충돌면을
넓히므로, 위 표→DIAGRAM 오탐과 근본적으로 동일한 이유로 이 Phase 범위에서는
손대지 않는다.
"""
