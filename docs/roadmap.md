...
| # | Task | 요구사항 ID | 의존 | ID |
|---|---|---|---|---|
| 1 | 프로젝트 스캐폴딩 (git init, venv, 폴더구조, pytest) | — | 없음 | `954d8b88` |
| 1b | 합성 테스트 픽스처 생성 유틸리티 (텍스트/도형/악보 왜곡 이미지 코드로 생성) | — | #1 | `ef4c4c61` |
| 2 | 공통 전처리 파이프라인 (원근보정/deskew/조명보정/업스케일) | PRE-1~5 | #1, #1b | `00a177ec` |
| 3 | 텍스트 OCR 처리기 (OCR + OCRmyPDF) — ⚠️Ghostscript 설치 필요(착수 직전, Tesseract/qpdf는 설치완료) | TXT-1,2 | #2 | `3770f54b` |
| 4 | PDF 조립 최소 구현 (단순 병합) | PDF-1 | #3 | `c48f1148` |
| 5 | 최소 GUI (입력/미리보기/저장, 처리 파이프라인은 QThread로 실행해 UI 비블로킹 보장) | GUI-1,2,4 | #4 | `bce4fa7d` |
| 6 | 텍스트 검수 UI | TXT-3 | #5 | `40e5540c` |
| 7 | Phase1 End-to-End 검증 | §9 Phase1 | #6 | `185b6d07` |
...
| # | Task | 요구사항 ID | 의존 | ID |
|---|---|---|---|---|
| 1 | 문서 유형 라우팅(router) 구현 | RT-1,2 | Phase1#7 | `7ef7d4ad` |
| 2 | 도형 선명화 | DIA-1 | #1 | `150b2fa8` |
| 3 | 도형 벡터화 옵션 + 한계 고지 | DIA-2,3 | #2 | `5b71299b` |
| 4 | GUI에 도형 처리 경로 연결 | DIA-3(UI) | #3 | `0d043c72` |
| 5 | Phase2 End-to-End 검증 | §9 Phase2/3 | #4 | `d2d67738` |

## Phase 3 — 악보 처리 (OMR)

| # | Task | 요구사항 ID | 의존 | ID |
|---|---|---|---|---|
| 1 | 악보 OMR 인식 (oemer 연동) | SCR-1 | Phase2#1 | `cfb53ee4` |
| 2 | 재조판 PDF 생성 (MuseScore 연동) — ⚠️사전 설치: MuseScore 4 | SCR-2 | #1 | `b8cba231` |
| 3 | 악보 오류 검수 경로 (외부 편집기 열기, 동일하게 MuseScore 설치 필요) | SCR-3 | #2 | `59367417` |
| 4 | GUI에 악보 처리 경로 연결 | — | #3 | `0deef3cf` |
| 5 | Phase3 End-to-End 검증 | §9 Phase2/3 | #4 | `96bc862b` |

## Phase 4 — GUI 고도화

| # | Task | 요구사항 ID | 의존 | ID |
|---|---|---|---|---|
| 1 | 수동 보정 (자르기/회전) | GUI-3(일부) | Phase2#5, Phase3#5 | `851d5923` |
| 2 | 도형/악보 검수 위젯 통합 | GUI-3(전체) | #1 | `32dc96c8` |
| 3 | 페이지 재정렬/삭제 | PDF-2 | #2 | `6a46cdf1` |
| 4 | 유형 자동 라우팅 정교화 | RT-1,2(고도화) | #3 | `9d32a092` |
| 5 | Phase4 End-to-End 검증 (혼합 워크플로우) | §9 Phase4 | #4 | `1abca8b7` |
...
| # | Task | 요구사항 ID | 의존 | ID |
|---|---|---|---|---|
| 1 | 로컬 저장 우선 보장 + 백업 설정 UI(기본 off) | BKP-1 | Phase4#5 | `90830178` |
| 2 | Supabase 업로드 구현 | BKP-2 | #1, #3 | `005aed8e` |
| 3 | 자격증명 관리 (.env) | BKP-4 | #1 | `84f668f8` |
| 4 | Phase5 End-to-End 검증 (오프라인 보장 포함) | BKP-3, §9 Phase5 | #2, #3 | `ab376901` |
...
## 진행 상황 (Phase1 착수 전 환경 준비)

Phase1-1(스캐폴딩) task 자체는 아직 미실행이지만, 사용자 요청으로 Phase1에 필요한 라이브러리/프로그램을 먼저 설치함. 실제로 설치해보며 발견한 것들을 Phase1-1/Phase1-3 task notes에 반영해둠(위 링크의 `get_task_detail`로 상세 확인 가능):

- venv는 **Python 3.12**로 생성(3.13은 PEP 667 때문에 basicsr 설치 자체가 깨짐 — 실제 재현 확인함). Phase1-1에서 pyproject.toml 작성 시 Python 버전 제약을 `>=3.12,<3.13`으로 명시할 것.
- basicsr는 설치되어도 import 시점에 `torchvision.transforms.functional_tensor`(최신 torchvision에서 삭제됨) 오류가 남아 있어 한 줄 패치가 필요함 — 현재 `.venv`에는 적용돼 있지만 재생성 시 다시 깨지므로 Phase1-1에서 재현 가능하게 기록 필요.
...
- 외부 프로그램: Tesseract 5.5.3·qpdf 12.3.2 설치 및 PATH 등록 완료. **Ghostscript는 winget 카탈로그에 없어 미설치 — Phase1-3 착수 직전에 수동 설치하기로 함(사용자 결정).**
