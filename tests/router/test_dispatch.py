...
텍스트 유형은 실제 `app.processors.text.process_image`로 위임되는지 확인하고
(Tesseract/Ghostscript/qpdf가 없으면 skip), 도형/악보 유형은 아직 처리기가
없으므로 조용히 무시되거나 텍스트로 폴백되지 않고 명확한 예외가 발생하는지 확인한다.
"""
...
@pytest.mark.parametrize("document_type", [DocumentType.DIAGRAM, DocumentType.SCORE])
def test_route_and_process_raises_for_unimplemented_processors(
    synthetic_text_photo: SyntheticPhoto, tmp_path, document_type: DocumentType
) -> None:
    """도형/악보 처리기가 아직 없으므로 조용히 무시하거나 텍스트로 폴백하면 안 된다."""
    preprocessed = run_pipeline(synthetic_text_photo.photo, PreprocessConfig())
    output_pdf = tmp_path / "output.pdf"

    with pytest.raises(UnsupportedDocumentTypeError):
        route_and_process(preprocessed, output_pdf, override=document_type)
